...
everything on the website with no coding required. Every field in the
editor now also has a short plain-English hint under it explaining
exactly where it shows up on the site.
...
- **The office address** — one field, under 🌐 Site-Wide Settings,
  controls the address text shown in every footer, the Contact page,
  AND automatically moves the interactive Google Map and every
  "Get Directions" button across the site. There is nothing else to
  update — no separate map link, no coordinates.
- **Testimonials** — the "What Our Clients Say" section on the Home,
  Enterprises, Associates, and Foundation pages is driven entirely
  from the CMS. It stays completely hidden on the live site until you
  add a first real quote, so visitors never see placeholder text.

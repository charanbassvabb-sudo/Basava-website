...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // (checked once, used below to skip purely-decorative animation work)
  // =========================
  const prefersReducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

  // =========================
  // ACCESSIBILITY: skip-to-content link
  // Added once here so every page gets it without editing each HTML file.
  // =========================
  ...
  // Paper (the "light-mode" class, historically) is now the primary,
  // default reading experience — the editorial look this site is built
  // around. Ink (dark) is there for anyone who prefers it and is
  // remembered once someone explicitly picks it.
  // =========================
  (function initTheme() {
  ...
  // #site-loader is static markup at the very top of <body> on every
  // page, so it's the first thing painted — visitors see the Basava
  // logo on a dark, on-brand background instead of a flash of a flat,
  // mismatched color while the page finishes loading. We fade it away
  // once this page is ready, and bring it back for a beat before
  // navigating to the next page, so every transition feels intentional.
  // =========================
  const siteLoader = document.getElementById("site-loader");
  if (siteLoader) {
    requestAnimationFrame(() => {
      requestAnimationFrame(() => { siteLoader.classList.add("hide"); });
    });
  ...
  progressBar.style.cssText = "position:fixed;top:0;left:0;height:2px;width:0%;background:var(--ed-accent,#e0895a);z-index:9999;transition:width 0.1s linear;pointer-events:none;";
  ...
  btt.style.cssText = "position:fixed;bottom:110px;right:28px;width:46px;height:46px;border-radius:3px;background:var(--ed-accent,#e0895a);color:var(--ed-on-accent,#17140f);border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:none;opacity:0;transform:translateY(20px);transition:opacity 0.35s ease,transform 0.35s ease,background 0.3s ease;z-index:9000;pointer-events:none;";
  ...
  btt.addEventListener("mouseenter", () => { btt.style.background = "var(--ed-accent-deep,#c76b3d)"; });
  btt.addEventListener("mouseleave", () => { btt.style.background = "var(--ed-accent,#e0895a)"; });
  ...
  // =========================
  // MOBILE ACTION BAR (Call / WhatsApp / Directions)
  // Fixed bottom bar shown on small screens only (see CSS). The Call and
  // Directions links carry data-cms-g attributes so the CMS loader fills
  // them in automatically from the phone number / address set in Admin →
  // Site-Wide Settings, exactly like every other data-cms-g element.
  // =========================
  const mobileBar = document.createElement("div");
  mobileBar.id = "mobile-action-bar";
  mobileBar.setAttribute("role", "navigation");
  mobileBar.setAttribute("aria-label", "Quick contact actions");
  mobileBar.innerHTML = `
    <a href="tel:+260966328089" data-cms-g="phone1" data-cms-kind="tel" aria-label="Call us">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
      <span>Call</span>
    </a>
    <a href="https://wa.me/260966328089" target="_blank" rel="noopener noreferrer" aria-label="Chat on WhatsApp">
    ...
    <a href="https://www.google.com/maps/dir/?api=1&destination=Plot%20No.%2020%2C%20Central%20Street%2C%20Chudleigh%2C%20Lusaka%2C%20Zambia" data-cms-g="address" data-cms-kind="mapdir" target="_blank" rel="noopener noreferrer" aria-label="Get directions">

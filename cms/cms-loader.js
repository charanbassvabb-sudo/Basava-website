...
(function () {
  ...
  function applyOne(el, val) {
    ...
    if (kind === "img") {
      ...
      // Embeds a live Google Map for whatever address string is in the CMS —
      // editors just change the "Address" text field and the map moves itself.
      el.src = "https://www.google.com/maps?q=" + encodeURIComponent(val) + "&output=embed";
    } else if (kind === "mapdir") {
      // "Get Directions" links built the same way — no separate map link to maintain.
      el.href = "https://www.google.com/maps/dir/?api=1&destination=" + encodeURIComponent(val);
    } else {
      el.textContent = val;
    }
  }
  ...
  function renderItem(type, item, container) {
    ...
    switch (type) {
      ...
      case "testimonial":
        var stars = Math.max(1, Math.min(5, parseInt(item.stars, 10) || 5));
        return (
          '<div class="' + itemClass + '"><div class="testimonial-stars">' + "★".repeat(stars) +
          '</div><p class="testimonial-quote">“' + esc(item.quote) + '”</p>' +
          '<p class="testimonial-author">' + esc(item.name) + "</p>" +
          '<p class="testimonial-role">' + esc(item.role) + "</p></div>"
        );
      case "event":
        return (
  ...
  function applyLists(data) {
    document.querySelectorAll("[data-cms-list]").forEach(function (el) {
      ...
      // Sections fed entirely from a CMS list (e.g. Testimonials) simply
      // disappear from the live site until an editor adds a first real
      // entry — no placeholder/fake content ever ships to visitors.
      var section = el.closest("section");
      if (items.length === 0) {
        if (section && el.hasAttribute("data-cms-hide-if-empty")) section.style.display = "none";
        el.innerHTML = "";
        return;
      }
